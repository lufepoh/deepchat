...
def debug_print(message):
    if debug_logging:
        ...
        logger.debug(f"[{timestamp}][{thread_name}] {message}")
...
async def control_handler(websocket):
    ...
    logger.info(f"Control client connected (ID: {client_id})")
    control_connections.add(websocket)
    try:
        async for message in websocket:
            debug_print(f"Received control message from client {client_id}: {message[:200]}...")
        ...
        logger.info(f"Control client disconnected (ID: {client_id}): {e}")
    except Exception as e:
        logger.error(f"Error in control handler for client {client_id}: {e}")
    finally:
        try:
            ...
            pass
        logger.info(f"Control client cleanup completed (ID: {client_id})")

async def data_handler(websocket):
    """WebSocket 데이터 핸들러"""
    global writechunks, wav_file
    client_id = id(websocket)
    logger.info(f"Data client connected (ID: {client_id})")
    ...
    try:
        async for message in websocket:
            if isinstance(message, bytes):
                ...
                logger.info(f"{bcolors.WARNING}Received non-binary message on data connection{bcolors.ENDC}")
    except websockets.exceptions.ConnectionClosed as e:
        logger.info(f"Data client disconnected (ID: {client_id}): {e}")
    except Exception as e:
        logger.error(f"Error in data handler for client {client_id}: {e}")
    finally:
        try:
            ...
            pass
        logger.info(f"Data client cleanup completed (ID: {client_id})")
    ...
    while True:
        ...
        for conn in data_connections:
            try:
                timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
                if extended_logging:
                    logger.debug(f"Sending message to client {id(conn)}: {message}")
                await conn.send(message)
            except websockets.exceptions.ConnectionClosed:
                disconnected_clients.add(conn)
            except Exception as e:
                logger.error(f"Error broadcasting to client {id(conn)}: {e}")
                disconnected_clients.add(conn)
        ...
        for conn in disconnected_clients:
            try:
                ...
                logger.info(f"Removed disconnected client {id(conn)} from data connections")
            except:
                pass
...
class ResourceMonitor:
    ...
    def collect_metrics(self):
        ...
        self.metrics['audio_queue_size'].append(audio_queue.qsize())
    ...
    def get_metrics(self):
        ...
        return {
            ...
            'audio_queue_size': self.metrics['audio_queue_size'][-1] if self.metrics['audio_queue_size'] else 0,
...
async def main_async():
    ...
    monitor_task = asyncio.create_task(resource_monitor.monitor())
    ...
    try:
        ...
        logger.info(f"Control server started on ws://0.0.0.0:{args.control}")
        logger.info(f"Data server started on ws://0.0.0.0:{args.data}")
        ...
        logger.error(f"Error: Could not start server on specified ports. It's possible another instance of the server is already running, or the ports are being used by another application.")
    except KeyboardInterrupt:
        logger.warning("Server interrupted by user, shutting down...")
    finally:
...
async def shutdown_procedure():
    ...
    if recorder:
        ...
        recorder.stop()
        recorder.shutdown()
        logger.info("Recorder shut down")
    ...
    logger.info("All tasks cancelled, closing event loop now.")
